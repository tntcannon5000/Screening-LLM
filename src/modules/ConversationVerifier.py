...
def process_qa_pair_noscrape(chat_log):
    # Set up OpenAI API (only if needed)
    os.environ['OPENAI_API_KEY'] = os.getenv("OPENAI_API_KEY")
    ...
    Use your own knowledge and expertise on the subject matter to get the accuracy of the answer. Provide a percentage accuracy score and also note down the places where the answer was inaccurate and give feedback for those places. Please keep in mind do not check for this:
    1. There may be some spelling errors or words being incorrectly transcribed due to this being a transcription of an audio interview. 
    2. Do not look into things the candidate did not answer if it was not asked in the question.
    3. Do not provide a summary at the end of each question-answer pair.
    """
    accuracy_prompt = ChatPromptTemplate.from_template(accuracy_template)
    ...
    for chat in chat_log:
        ...
        # Invoke the LLM to check the accuracy
        feedback = llm(accuracy_prompt.format(
            question=question,
            answer=answer,
            context=q_a_pairs  # Optional: if you want to include accumulated Q&A pairs
        ))
